define(function (require, exports, module) {
    ...
    (function ($) {
         ...
         */
        var DataProxy = ChineseRegion.DataProxy = function (remote, init, dataSource) {
            ...
            for (var i= 0, l = this.dataSource.length; i<l; i++){
                this.dataCache['cached_'+this.dataSource[i]["i"]] =  this.dataSource[i]["c"];
            }
//            this._initDataSource(this.dataSource);
        ...
        DataProxy.prototype = {
            ...
            _initDataSource:function(arr){
                for (var i= 0, l = arr.length; i<l; i++){
                    if(!this.dataCache['cached_'+arr[i]["i"]] && arr[i]["c"] && arr[i]["c"].length){
                        this.dataCache['cached_'+arr[i]["i"]] =  arr[i]["c"];
                        this._initDataSource(arr[i]["c"]);
                    }
            ...
            load: function (id, f) {
                ...
                var ds = (id == "all" ? this.dataSource : this.dataCache[cache_id])

                ChineseRegion._caches[cache_id] = ds;
                f(new RegionCollection(ds, cache_id), self);
            },
        ...
        RegionPicker.prototype = {
            ...
            _pickedCollections: function (regions) {
                ...
                var collections = [];
                if (regions[0]) {
                    collections.push(this.proxy.collection('all').collection);
                }
                if (regions[1]) {
